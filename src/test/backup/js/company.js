        ...
        success: function (result) {
            ...
            if(page_count>0){
                ...
                for(var i=0;i<result.data.length;i++){
                    if(result.data[i].p_qualifications===null){
                        createMyHtmlData(result.data[i]);
                        $("#itemList").append($("#search-show-list").html());
                    }else{
                        createMyHtmlData(result.data[i]);
                        var len=result.data[i].p_qualifications.indexOf(",");
                        var url=imgPath+result.data[i].p_qualifications.substring(0, len)
                        $("#search-show-list .items  #img").attr("src",url);
                        $("#itemList").append($("#search-show-list").html());
                    }
