...
<div class="hniois-table">
    <form action="outfile/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <c:if test="${fn:contains(QX,'outfile/add')}">
                ...
                <a id="hniois-export" onclick="doExport('outfile/doExport')" class="layui-btn layui-btn-sm layui-bg-orange" href="javascript:;" >
            ...
            <div class="hniois-search-body">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="government" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
            </div>
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'outfile/update')}">
        <a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
    </c:if>
    <c:if test="${fn:contains(QX,'outfile/delete')}">
        <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="del">删除</a>
    </c:if>
    <%--<c:if test="${fn:contains(QX,'outfile/print')}">
        <a class="layui-btn layui-btn-xs layui-btn-primary" lay-event="print">打印</a>
    </c:if>--%>
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            ...
            cols: [[
             ...
             //   {field:'farm_id', title: '基地ID',align:'center'},
                {field:'pro_name', title: '产品名称',align:'center',width:120},
                {field:'pro_unit',  title: '产品规格',edit:'text',align:'center',width:100},
                {field:'pro_num',  title: '产品数量',edit:'text',align:'center',width:100},
                {field:'report_num',  title: '检测编号',align:'center',width:140},
                {field:'government',  title: '审批机构',align:'center',width:200},
                {field:'approver',  title: '审批人',align:'center',width:100},
                {field:'out_time',  title: '发证时间',align:'center',width:150},
                {field:'print_time',  title: '打印日期',align:'center',width:150},
                {field:'company',  title: '生产基地',align:'center',width:200},
                {field:'phone',  title: '联系电话',align:'center',width:120},
                {field:'address',  title: '地址',align:'center',width:300},
                {field:'crop_img',  title: '图片',width:60,align:'center',templet:function(d){
                        //打药多张图片，默认显示第一张
                ...
                {fixed: 'right', align:'center', width:200,toolbar: '#editBar'}
            ]],
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "outfile/toEdit.do?fid="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
    ...
    //三品一标照片上传
    function toChangePosionImgs(id){
        var layer = layui.layer ;
        layer.open({
            title : "打药图片",
            type: 2,
            ...
            maxmin: true,
            skin: 'layui-layer-rim', //加边框
            content: "outfile/toEditImgs.do?fid="+id,
            area: ['650px', '80%']
        });
    }
