...
//    首页  企业基地用
    String show_page3 = "system/index/show3";
        ...
        if(user!=null){
            if("0".equals(user.getState())) {
                if(Const.ADMINISTRATOR.equals(user.getBz()) || Const.OUTFIT_ADMIN.equals(user.getBz()) ||
                    ...
                    obj.put("state","error");
                    obj.put("msg","登陆账号信息异常,请联系系统管理员！！");
                }
            } else {
                obj.put("state","error");
                obj.put("msg","登录账号信息状态未启用,请联系系统管理员！！");
            }
        }else{
            obj.put("state","error");
    ...
    public ModelAndView main() {
        ...
        try {
            ...
            if(user != null){
                Menu p = new Menu();
                p.setMenu_ids("#".equals(user.getRights())? "" : user.getRights());
...
//        超级管理员 或  监管机构用户
        ...
        } else if(Const.PROVIDER_ADMIN.equals(user.getBz())) {
            mv.setViewName(show_page3);
        } else {
            mv.addObject("error", "登陆用户异常,请联系管理员");
