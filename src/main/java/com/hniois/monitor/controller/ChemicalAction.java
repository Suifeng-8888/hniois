...
public class ChemicalAction extends BaseController{
    ...
    @ResponseBody
    public String delete(Chemical e, HttpServletRequest request) throws Exception {
        JSONObject obj = getMsg();//返回信息
        if(StringUtils.isNotEmpty(e.getId())){
            chemicalService.newDel(e,request);
        }else{
            setMsg(obj,"error","ID不能为空");
        }
        return obj.toString();
    ...
    @ResponseBody
    public String deletes(Chemical e,HttpServletRequest request) throws Exception {
        JSONObject obj = getMsg();//返回信息
        if(e.getData().size()>0){
            chemicalService.newDelBatch(e,request);
        }else{
