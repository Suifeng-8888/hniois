...
public class ReportAction extends BaseController{
    ...
    @ResponseBody
    public String delete(Report e,HttpServletRequest request)throws Exception{
        JSONObject obj=getMsg();
        if(StringUtils.isNotEmpty(e.getRid())) {
            reportService.newDel(e,request);
        }else {
            setMsg(obj, "error", "ID不能为空");
        }
        return obj.toString();
    ...
    @ResponseBody
    public String deletes(Report e,HttpServletRequest request)throws Exception{
        JSONObject obj=getMsg();
        if(e.getData().size()>0){
            reportService.newDelBatch(e,request);
        }else{
