...
<%--检测报告图片信息--%>
<div style="display: none" id="s_img">${seed.s_img}</div>
<div style="display: none" id="s_descr">${seed.s_descr}</div>
</body>
...
<script>
    ...
    //初始化图片
    function init_IMG(){
        var img_hmtl = $("#s_img").html()||"";
        var imgs = img_hmtl.split(",");
        var descr_html=$("#s_descr").html()||"";
        var des_html=descr_html.split(",");
        for(var i=0;i<imgs.length;i++){
            if($.trim(imgs[i])&&$.trim(des_html[i])){
                var result=des_html[i].split("@");
                add_IMG2(i,imgs[i],"old",result);
            }
        }
    }
     ...
     * */
    function add_IMG(index,url,cls,des){
        $('#upload_img_list').append(
            ...
            '<i class="close layui-icon" onclick=delete_IMG(this,"' + index + '","'+url+'") ></i>' +
            '</div>' +
            '<img src="' + url + '" class="img" onclick=downLoad("'+url+'")>' +
            '<input type="hidden" class="img_' + cls + '" name="imgs" value="' + url + '" />' +
            '<input name="mark" type="hidden" value="<%=uuid%>"/>'+
            '<input type="text"  name="desc" autocomplete="off" style="display: block;width: 196px;background-color: white;" value="'+des+'" onchange="updateDesc()" />'+
            '</dd>');
    }


    //初始化加载时调用
    function add_IMG2(index,url,cls,result){
        $('#upload_img_list').append(
            '<dd class="item_img" id="img_' + index + '">' +
            '<div class="operate"><i class="toleft layui-icon"></i>' +
            '<i class="toright layui-icon"></i>' +
            ...
            '<img src="' + url + '" class="img" onclick=downLoad("'+url+'")>' +
            '<input type="hidden" class="img_' + cls + '" name="imgs" value="' + url + '" />' +
            '<input name="mark" type="hidden" value="'+result[0]+'"/>'+
            '<input type="text"  name="desc" autocomplete="off" style="display: block;width: 196px;background-color: white;" value="'+result[1]+'" onchange="updateDesc()" />'+
            '</dd>');
    }

    //更改图片下面的文字说明
    function updateDesc(){
        updateSchoolData("描述已修改");
    }

    //删除图片
    function delete_IMG(o,divs,url) {
        var temp_desc=$(o).parent().parent().find("input[name='mark']").val();
        var des=$(o).parent().parent().find("input[name='desc']").val();
        $.ajax({
            url: "seed/updateDelet.do",
            data: {
                s_id:$("#key").val(),
                s_path:url , //删除路径
                des_temp:temp_desc+"@"+des
            },
            type: "post",
            dataType:'json',
    ...
    function updateSchoolData(msg){
        ...
        $(".item_img").each(function () {
            descr+=$(this).find("input[name='mark']").val()+"@"+$(this).find("input[name='desc']").val()+",";
        })
        $.ajax({
            url: "seed/update.do",
            data: {
