    ...
    <form action="Buy/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <a id="hniois-export" onclick="doExport('buy/doExport?msg=1')" class="layui-btn layui-btn-sm layui-bg-orange" href="javascript:;" >
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            elem: '#table',
            url:'buy/outSupList.do',
            limits:[10],
            cols: [[
                {type:'checkbox'},
               // {field:'id',  title: '序号'},
                {field:'b_provider',  title: '供应商'},
                {field:'b_name',  title: '采购产品'},
                {field:'cost',  title: '采购总价(¥)'},
    ...
    function initChart11(d){
        ...
        for(var i=0;i<d.data.length;i++){
            ...
            dataName.push(d.data[i].b_provider);
        }
        var  option = {
            title : {
                text: '最近半年采购供应商商品费用',
    ...
    function initChart22(d){
        ...
        for (var i = 0; i < obj.length; i++) {
            arrNum.push({"value": objNum[i].cost,"name":obj[i].b_provider});
        }
        var  option = {
            title : {
                text: '采购供应商费用对比',
