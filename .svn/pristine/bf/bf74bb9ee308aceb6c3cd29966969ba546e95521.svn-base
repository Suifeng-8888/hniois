...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            elem: '#table',
            url:'vaccine/list.do?breed_id='+'${breed.breed_id}',
            limits:[10],
            cols: [[
                {type:'checkbox'},
             //   {field:'va_id',  title:'疫苗编号'},
                {field:'var_name', title: '疫苗名称'},
                {field:'var_num', title: '注射方法'},
                {field:'var_time',  title: '注射时间',edit:'text'},
                {field:'var_doctor',  title: '注射人',edit:'text'},
        ...
        table.on('tool(lay_table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            var data = obj.data //获得当前行数据
             ...
             if(layEvent === 'edit'){
                toEdit(data.va_id,"${breed.breed_id}")
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "vaccine/toEdit.do?va_id="+id+"&breed_id="+code,
            area: ['650px', '80%']
        });
    }
