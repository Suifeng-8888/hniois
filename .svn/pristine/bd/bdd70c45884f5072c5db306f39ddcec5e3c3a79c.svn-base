...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            ...
            cols: [[
                {type:'checkbox'},
              //  {field:'crop_id',  title: '土地编号'},
              //  {field:'farm_id',  title: '作物编号'},
                {field:'crop_name',  title: '作物名称'},
                {field:'crop_mark',  title: '产品标记'},
                {field:'crop_type',  title: '产品类别'},
                {field:'crop_desc',  title: '作物简介'}
    ...
    function sendMsg(){
        ...
        if (d.data.length === 0) {
            layui.layer.msg('请选择农作物');
            return;
        }
        if (d.data.length > 1) {
            layui.layer.msg('每次只能选择一种作物');
            return;
        }
        var data = d.data,
            ids=[],
        ...
        for(var i=0;i<data.length;i++){
            names.push(data[i].crop_name);
            ids.push(data[i].crop_id);
            marks.push(data[i].crop_mark);
        }
        parent.$('#cp').val(names.toString());
        parent.$('#cId').val(ids.toString());
        parent.$('#scode').val(marks.toString());
